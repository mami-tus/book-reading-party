...
func TestFindEntries(t *testing.T) {
	// httptest.NewServer関数を使用してテスト用のHTTPサーバーを作成しています。
	// この関数は、HTTPリクエストを処理するためのハンドラ関数を引数として受け取ります。
	// このハンドラ関数は、HTTPレスポンスライタ(w http.ResponseWriter)とHTTPリクエスト(r *http.Request)を引数として受け取り、サーバーのレスポンスを生成します。
